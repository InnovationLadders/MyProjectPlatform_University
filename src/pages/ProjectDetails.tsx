...
import {
  ...
  Download, Plus, Upload, UserMinus
} from 'lucide-react';
import { getProjectById, getProjectStudents, getProjectTasks, db, firestoreDoc, removeStudentFromProject } from '../lib/firebase';
            ...
            {projectStudents.length > 0 ? (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {projectStudents.map((studentRecord) => (
                  <div key={studentRecord.id} className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition-all relative group">
                    {canEditProject && (
                      <button
                        onClick={() => handleRemoveStudentClick(studentRecord)}
                        className="absolute top-3 left-3 p-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition-colors opacity-0 group-hover:opacity-100"
