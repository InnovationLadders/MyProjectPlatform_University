...
export const useProjects = () => {
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      const projectsWithSchoolNames = await Promise.all(
        userProjects.map(async (project) => {
          ...
          try {
            ...
            console.log('Student count for project', project.id, ':', studentCount);
          ...
          try {
            ...
            console.log('Task counts for project', project.id, '- Total:', totalTasks, 'Completed:', completedTasks);
          } catch (error) {
      ...
      console.log('Final projects with school names and counts:', projectsWithSchoolNames.map(p => ({
        id: p.id,
        title: p.title,
