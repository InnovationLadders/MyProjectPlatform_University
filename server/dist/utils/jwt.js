...
export async function initializeKeys() {
    ...
    try {
        ...
        publicKey = await jose.importSPKI(await exportPublicKey(privateKeyPem), 'RS256');
...
export async function verifyJWT(token, jwks) {
    const JWKS = jose.createRemoteJWKSet(new URL(jwks));
    const { payload } = await jose.jwtVerify(token, JWKS);
    return payload;
}
export async function verifyJWTWithKey(token, publicKeyToUse) {
